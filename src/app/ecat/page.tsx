...
export const metadata = { title: 'ECAT Chemistry' };

export default function EcatPage() {
  const icons = [Atom, FlaskConical, Beaker];
  return (
    <div className="container-wide py-8">
      <Breadcrumbs items={[{ label: 'Home', href: '/' }, { label: 'ECAT' }]} />
      <div className="mt-6 mb-8">
        <span className="eyebrow mb-2">Engineering Entry Test</span>
        <h1 className="text-3xl sm:text-4xl font-serif font-semibold text-ink">ECAT Chemistry</h1>
        <p className="mt-3 text-ink-muted max-w-2xl leading-relaxed">Comprehensive ECAT Chemistry preparation covering Physical, Inorganic and Organic Chemistry with numerical problem-solving focus.</p>
      </div>
      <div className="grid gap-5 md:grid-cols-3">
        {ecatGroups.map((group, i) => {
          const Icon = icons[i] || Atom;
          return (
            <Link key={group.slug} href={`/ecat/${group.slug}`} className="card-hover p-6 group">
